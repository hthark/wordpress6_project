...
class GoogleSitemapGeneratorLoader {
	...
	public static function RegisterPluginLinks($links, $file) {
		...
		if($file == $base) {
			...
			$links[] = '<a href="http://www.ijunkey.com/">' . __('FAQ', 'sitemap') . '</a>';
			$links[] = '<a href="http://www.ijunkey.com/">' . __('Support', 'sitemap') . '</a>';
			$links[] = '<a href="http://www.ijunkey.com/">' . __('Work', 'sitemap') . '</a>';
		}
		return $links;
	}
	...
	public static function CallHtmlShowHelpList() {
		...
		if(is_object($screen) &&  $screen->id == $id) {
			...
			$links = array(
				__('Plugin Homepage', 'sitemap') => 'http://www.ijunkey.com/',
				__('My Sitemaps FAQ', 'sitemap') => 'http://www.ijunkey.com/'
			);
